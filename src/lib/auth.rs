...
impl Auth {
    pub fn token(&self, secret: &[u8]) -> String {
        jwt::encode(&jwt::Header::default(), self, &jwt::EncodingKey::from_secret(secret)).expect("jwt")
    }
}
...
fn decode_token(token: &str, secret: &[u8]) -> Option<Auth> {
    ...
    jwt::decode(token, &jwt::DecodingKey::from_secret(secret), &Validation::new(Algorithm::HS256))
        .map_err(|err| {
